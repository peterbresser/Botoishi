...
mapBounds = map.getBounds();
 ...
 ifmoved=1;
    ...
    if (feature.geometry.type === \"Point\"  && feature.properties.linkedto==\"toilet\") {
     ...
     visibility=1;
        ...
        if(visibility <=zoomlevel){
                 ...
                 // console.log( ' -   '+visibility+' -- '+zoomlevel);
                if(linename==0)
                     linename='';   
...
// window['marker'+1]=new L.Marker([coords[0], coords[1] ], {icon:HighLiteIcon}).addTo(markerGroup);;
                    window['marker'+1]. setLatLng([lat,lon ]).bindPopup(popupText ,{ autoClose: false,   'className': 'stylePopup' ,  closeButton: true, closeOnClick : true ,autoPan:false });;                         
...
map.setView([35.6596355,139.691524],14); 
 
         var HighLiteIcon= L.icon({
         iconUrl: 'images/mkothers.png',
         shadowUrl: 'images/mkothers.png',
         iconSize:    [11,11],
         shadowSize:   [1,1]
                }); 
            const latLng = L.latLng(coords[0], coords[1]);
...
"
                    ...
                    window['marker'+1]. setLatLng([coords[0], coords[1] ]).bindPopup(popupText ,{closeOnClick: false,autoClose: false,  autoPan:false, 'className': 'stylePopup' ,    offset: [-100,30], closeButton: true, 'closeOnClick' : true  });;                         
                    window['marker'+1].on('onclick', function (e) {   closePopupTimeout = setTimeout(() => { this.closePopup();}, 1500);}); 
           //   }
            }  
          }
