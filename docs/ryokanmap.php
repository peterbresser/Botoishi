...
echo "
 var markers = [";
$query=mysqli_query($japansql ,"select id,location_name, latitude, longitude, description, web_page, marker from locations where type ='hotel' and marker ='ryokan'  ". $locationtype."  and latitude !=''");
while($result=mysqli_fetch_array($query,MYSQLI_ASSOC)){
    ...
    $diplay="<table width=100%> <tr><td>". $directions."</td><td> </td> <td>".$website."</td><td> </td></tr></table>";   if(isset($_GET['id'])) {
        $idlatlon=$result['latitude'].",".$result['longitude']."";
    }
    if(strlen($data)>0)
        $data.= ",[".$result['latitude'].",".$result['longitude'].",'".$result['location_name']."</b>','". $diplay."','".$result['marker']."'  ]";
    else
        $data=   "[".$result['latitude'].",".$result['longitude'].",'".$result['location_name']."</b>','". $diplay."','".$result['marker']."'  ]";
...
if(isset($_GET['stations'])) {
    ...
    foreach ($stationhotels as $id => $stationdata) {
        ...
        if (strlen($stationmerkerdata) > 0)
            $stationmerkerdata .= ",[" . $stationdata['latitude'] . "," . $stationdata['longitude'] . ",'<div class=\"text-center \">" . $hoteldata['location_name'] . "</div><br>" . $stationdata['description'] . "','" . $diplay . "','" . $stationdata['marker'] . "','" . $hoteldirections . "' ]";
        else
            $stationmerkerdata = "[" . $stationdata['latitude'] . "," . $stationdata['longitude'] . ",'<div class=\"text-center \">" . $hoteldata['location_name'] . "</div><br>" . $stationdata['description'] . "','" . $diplay . "','" . $stationdata['marker'] . "','" . $hoteldirections . "' ]";
    }
