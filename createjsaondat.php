...
include  "japan.inc";
           $query=mysqli_query($japansql ,"select  *   from company    ");
			...
			if(substr_count($result['company_name'],"Co., Ltd.")>0)
#				$result['company_name']=str_replace("Co., Ltd.","Company",$result['company_name']);
#			mysqli_query($japansql ,"update company set company_name ='". $result['company_name']."' where id =".$result['id']." ");
#echo "update company set company_name ='". $result['company_name']."' where id =".$result['id']."\n";
...
"type": "FeatureCollection",
 "features":[';
    $query=mysqli_query($japansql ,"select   longitude, latitude ,ename,jname,address ,stations.id as id , stations.line_id as lineid, company_id, line_name,zoomlevel,linked  from stations inner join japan.jplines jp  on  stations.line_id =jp.line_id where  active =1 and latitude!='0' and LENGTH(address) >1 ");
while($result=mysqli_fetch_array($query,MYSQLI_ASSOC)){
...
if(substr_count($result['line_name'],"(")>0)
    ...
    if(!array_key_exists($result['latitude'],$checked)){
		$checked[$result['latitude']]=array('latitude'=>$result['latitude'], 'longitude'=> $result['longitude'],'read'=>0.0000022);;
...
"properties": {"ename":"'.$result['ename'].'", "jname":"'.$result['jname'].'","address":"'.$result['address'].'", "lineid":"'.$result['lineid'].'","line_name":"'.$result['line_name'].'","display":"'.$display.'","marker":"'.$marker.'","mtype":"0","traintype":"0","visibility":"'.$result['zoomlevel'].'","id":"'.$result['id'].'","stationlocation":"1","linkedto":"'.$result['linked'].'"}
	 ...
	 $checked[$result['latitude']]['read']+=0.000022;

	$checked[$result['latitude']]['latitude']=$checked[$result['latitude']]['latitude']+ $checked[$result['latitude']]['read'];

	$checked[$result['latitude']]['longitude']+$checked[$result['longitude']]['read'];
	

  $data.= '
,{ "type": "Feature",
"geometry": {"type": "Point", "coordinates":['.$checked[$result['latitude']]['latitude'].','.$checked[$result['latitude']]['longitude'].']},
"properties": {"ename":"'.$result['ename'].'", "jname":"'.$result['jname'].'","address":"'.$result['address'].'", "lineid":"'.$result['lineid'].'","line_name":"'.$result['line_name'].'","display":"'.$display.'","marker":"'.$marker.'","mtype":"0","traintype":"0","visibility":"'.$result['zoomlevel'].'","id":"'.$result['id'].'","stationlocation":"1","linkedto":"'.$result['linked'].'"}
}';
...
#$result['location_name']=ucfirst(strtolower($result['location_name']));
$converted_string = mb_convert_case($result['location_name'], MB_CASE_TITLE, "UTF-8");
$result['location_name']=$converted_string ;
